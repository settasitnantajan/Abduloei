...
export function generateConfirmationPrompt(commandType: string, title: string): string {
  ...
  if (commandType === 'delete_all') {
    return `⚠️ ยืนยันจะลบรายการทั้งหมด (นัดหมาย, งาน, บันทึก, กิจวัตร) ไหมคะ?\n\nการลบจะย้อนกลับไม่ได้นะคะ\n\n💬 พิมพ์ "ใช่" เพื่อยืนยันลบ หรือ "ไม่" เพื่อยกเลิก`
  }

  if (commandType.startsWith('delete_')) {
    const deleteMap: Record<string, string> = {
      delete_event: 'นัดหมาย', delete_task: 'งาน', delete_note: 'บันทึก',
      delete_routine: 'กิจวัตร', delete_monthly_routine: 'กิจวัตรรายเดือน',
    }
    const typeName = deleteMap[commandType] || 'รายการ'
    return `⚠️ ยืนยันจะลบ${typeName} "${title}" ไหมคะ?\n\nการลบจะย้อนกลับไม่ได้นะคะ\n\n💬 พิมพ์ "ใช่" เพื่อยืนยันลบ หรือ "ไม่" เพื่อยกเลิก`
  }

  if (commandType.startsWith('edit_')) {
    const editMap: Record<string, string> = {
      edit_event: 'นัดหมาย', edit_task: 'งาน', edit_note: 'บันทึก',
      edit_routine: 'กิจวัตร', edit_monthly_routine: 'กิจวัตรรายเดือน',
    }
    const typeName = editMap[commandType] || 'รายการ'
    return `ยืนยันแก้ไข${typeName} "${title}" นี้ไหมคะ?\n\n💬 พิมพ์ "ใช่" เพื่อยืนยัน หรือ "ไม่" เพื่อยกเลิก`
  }

  const typeName = createMap[commandType] || 'คำสั่ง'
